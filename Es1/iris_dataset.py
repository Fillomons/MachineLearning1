...
fromWeb = True
if fromWeb:
    os.environ['KAGGLE_USERNAME'] = 'fillomons' # YOUR OWN USERID
    os.environ['KAGGLE_KEY'] = '48874fb34508da1e0544ce700a70da1e' # YOUR OWN KEY
    ...
    for target, target_name in enumerate(targets): # for each species
        ...
        ax1.plot(X_plot.iloc[:,0], X_plot.iloc[:,1], linestyle='none',
        marker='o', label=target_name)
        ax2.plot(X_plot.iloc[:,2], X_plot.iloc[:,3], linestyle='none',
        marker='o', label=target_name)
